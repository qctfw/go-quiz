...
return currentSteps.push(step)};var deleteQueueStep=function deleteQueueStep(index){if(typeof currentSteps[index]!=='undefined'){currentSteps.splice(index,1)}};var setDefaults=function setDefaults(userParams){if(!userParams||(typeof userParams==='undefined'?'undefined':_typeof(userParams))!=='object'){return error('the argument for setDefaults() is required and has to be a object')}
showWarningsForParams(userParams);for(var param in userParams){if(isValidParameter(param)){globalState.popupParams[param]=userParams[param]}}};var resetDefaults=function resetDefaults(){globalState.popupParams=_extends({},defaultParams)};var showLoading=function showLoading(){var popup=getPopup();if(!popup){SweetAlert('')}
...
function getInput(inputType){var domCache=this._domCache;inputType=inputType||this.params.input;if(!inputType){return null}
...
function enableButtons(){this._domCache.confirmButton.disabled=!1;this._domCache.cancelButton.disabled=!1}
function disableButtons(){this._domCache.confirmButton.disabled=!0;this._domCache.cancelButton.disabled=!0}
function enableConfirmButton(){this._domCache.confirmButton.disabled=!1}
function disableConfirmButton(){this._domCache.confirmButton.disabled=!0}
...
states.previousActiveElement=document.activeElement;if(onOpen!==null&&typeof onOpen==='function'){setTimeout(function(){onOpen(popup)})}};function _main(userParams){var _this=this;showWarningsForParams(userParams);var params=this.params=_extends({},globalState.popupParams,userParams);setParameters(params);Object.freeze(params);var domCache=this._domCache={popup:getPopup(),container:getContainer(),content:getContent(),actions:getActions(),confirmButton:getConfirmButton(),cancelButton:getCancelButton(),closeButton:getCloseButton(),validationError:getValidationError(),progressSteps:getProgressSteps()};var constructor=this.constructor;return new Promise(function(resolve,reject){var succeedWith=function succeedWith(value){constructor.closePopup(params.onClose,params.onAfterClose);if(params.useRejections){resolve(value)}else{resolve({value:value})}};var dismissWith=function dismissWith(dismiss){constructor.closePopup(params.onClose,params.onAfterClose);if(params.useRejections){reject(dismiss)}else{resolve({dismiss:dismiss})}};var errorWith=function errorWith(error$$1){constructor.closePopup(params.onClose,params.onAfterClose);reject(error$$1)};if(params.timer){domCache.popup.timeout=setTimeout(function(){return dismissWith('timer')},params.timer)}
...
domCache.closeButton.onclick=function(){dismissWith(constructor.DismissReason.close)};if(params.toast){domCache.popup.onclick=function(e){if(params.showConfirmButton||params.showCancelButton||params.showCloseButton||params.input){return}
constructor.closePopup(params.onClose,params.onAfterClose);dismissWith(constructor.DismissReason.close)}}else{var ignoreOutsideClick=!1;domCache.popup.onmousedown=function(){domCache.container.onmouseup=function(e){domCache.container.onmouseup=undefined;if(e.target===domCache.container){ignoreOutsideClick=!0}}};domCache.container.onmousedown=function(){domCache.popup.onmouseup=function(e){domCache.popup.onmouseup=undefined;if(e.target===domCache.popup||domCache.popup.contains(e.target)){ignoreOutsideClick=!0}}};domCache.container.onclick=function(e){if(ignoreOutsideClick){ignoreOutsideClick=!1;return}
if(e.target!==domCache.container){return}
if(callIfFunction(params.allowOutsideClick)){dismissWith(constructor.DismissReason.backdrop)}}}
...
constructor.clickConfirm();e.preventDefault()}}else if(e.key==='Tab'){var targetElement=e.target||e.srcElement;var focusableElements=getFocusableElements(params.focusCancel);var btnIndex=-1;for(var _i2=0;_i2<focusableElements.length;_i2++){if(targetElement===focusableElements[_i2]){btnIndex=_i2;break}}
if(!e.shiftKey){setFocus(btnIndex,1)}else{setFocus(btnIndex,-1)}
e.stopPropagation();e.preventDefault()}else if(arrowKeys.indexOf(e.key)!==-1){if(document.activeElement===domCache.confirmButton&&isVisible(domCache.cancelButton)){domCache.cancelButton.focus()}else if(document.activeElement===domCache.cancelButton&&isVisible(domCache.confirmButton)){domCache.confirmButton.focus()}}else if((e.key==='Escape'||e.key==='Esc')&&callIfFunction(params.allowEscapeKey)===!0){dismissWith(constructor.DismissReason.esc)}};if(params.toast&&globalState.windowOnkeydownOverridden){window.onkeydown=globalState.previousWindowKeyDown;globalState.windowOnkeydownOverridden=!1}
if(!params.toast&&!globalState.windowOnkeydownOverridden){globalState.previousWindowKeyDown=window.onkeydown;globalState.windowOnkeydownOverridden=!0;window.onkeydown=handleKeyDown}
_this.enableButtons();_this.hideLoading();_this.resetValidationError();if(params.input){addClass(document.body,swalClasses['has-input'])}
...
if(params.input==='select'||params.input==='radio'){var processInputOptions=function processInputOptions(inputOptions){return populateInputOptions(formatInputOptions(inputOptions))};if(isThenable(params.inputOptions)){constructor.showLoading();params.inputOptions.then(function(inputOptions){_this.hideLoading();processInputOptions(inputOptions)})}else if(_typeof(params.inputOptions)==='object'){processInputOptions(params.inputOptions)}else{error('Unexpected type of inputOptions! Expected object, Map or Promise, got '+_typeof(params.inputOptions))}}else if(['text','email','number','tel','textarea'].indexOf(params.input)!==-1&&isThenable(params.inputValue)){constructor.showLoading();hide(input);params.inputValue.then(function(inputValue){input.value=params.input==='number'?parseFloat(inputValue)||0:inputValue+'';show(input);_this.hideLoading()}).catch(function(err){error('Error in inputValue promise: '+err);input.value='';show(input);_this.hideLoading()})}
...
if(!(this instanceof SweetAlert)){return new(Function.prototype.bind.apply(SweetAlert,[null].concat(args)))()}
currentInstance=this;this._promise=this._main(this.constructor.argsToParams(args))}
SweetAlert.prototype.then=function(onFulfilled,onRejected){return this._promise.then(onFulfilled,onRejected)};SweetAlert.prototype.catch=function(onRejected){return this._promise.catch(onRejected)};SweetAlert.prototype.finally=function(onFinally){return this._promise.finally(onFinally)};_extends(SweetAlert.prototype,instanceMethods);_extends(SweetAlert,staticMethods);Object.keys(instanceMethods).forEach(function(key){SweetAlert[key]=function(){if(currentInstance){var _currentInstance;return(_currentInstance=currentInstance)[key].apply(_currentInstance,arguments)}}});SweetAlert.DismissReason=DismissReason;SweetAlert.noop=function(){};SweetAlert.version=version;SweetAlert.default=SweetAlert;if(typeof window!=='undefined'&&_typeof(window._swalDefaults)==='object'){SweetAlert.setDefaults(window._swalDefaults)}
return SweetAlert})));if(typeof window!=='undefined'&&window.Sweetalert2){window.swal=window.sweetAlert=window.Swal=window.SweetAlert=window.Sweetalert2}
